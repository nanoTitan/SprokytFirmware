...
void dwm_init(SPI_HandleTypeDef* spiHandle)
{
	LMH_Init(spiHandle);
}

int dwm_pos_set(SPI_HandleTypeDef* spiHandle, dwm_pos_t* pos)
{
	...
	*(uint32_t*)(tx_data+tx_len) = pos->z;
	...
	LMH_Tx(spiHandle, tx_data, &tx_len);      
	return LMH_WaitForRx(spiHandle, rx_data, &rx_len, 3);
}

int dwm_pos_get(SPI_HandleTypeDef* spiHandle, dwm_pos_t* p_pos)
{
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;	
	...
	tx_data[tx_len++] = 0;   
	LMH_Tx(spiHandle, tx_data, &tx_len);   
	if(LMH_WaitForRx(spiHandle, rx_data, &rx_len, 18) == RV_OK)
	{
		data_cnt = 5;
...
int dwm_upd_rate_set(SPI_HandleTypeDef* spiHandle, uint16_t ur, uint16_t ur_static)
{
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;
...
int dwm_upd_rate_get(SPI_HandleTypeDef* spiHandle, uint16_t *ur, uint16_t *ur_static)
{
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;
	...
	tx_data[tx_len++] = 0;   
	LMH_Tx(spiHandle, tx_data, &tx_len);   
	if(LMH_WaitForRx(spiHandle, rx_data, &rx_len, 9) == RV_OK)
	{
		data_cnt = 5;
...
int dwm_cfg_get(SPI_HandleTypeDef* spiHandle, dwm_cfg_t* cfg)
{
	...
	tx_data[tx_len++] = 0;   
	LMH_Tx(spiHandle, tx_data, &tx_len);   
	if(LMH_WaitForRx(spiHandle, rx_data, &rx_len, 7) == RV_OK)
	{
...
int dwm_sleep(SPI_HandleTypeDef* spiHandle)
{
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;
...
int dwm_loc_get(SPI_HandleTypeDef* spiHandle, dwm_loc_data_t* loc)
{ 
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;
	...
	tx_data[tx_len++] = 0;   
	LMH_Tx(spiHandle, tx_data, &tx_len);   
	if(LMH_WaitForRx(spiHandle, rx_data, &rx_len, DWM1001_TLV_MAX_SIZE) == RV_OK)
	{
...
int dwm_baddr_set(SPI_HandleTypeDef* spiHandle, dwm_baddr_t* p_baddr)
{
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;
...
int dwm_baddr_get(SPI_HandleTypeDef* spiHandle, dwm_baddr_t* p_baddr)
{
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;
	...
	tx_data[tx_len++] = 0;   
	LMH_Tx(spiHandle, tx_data, &tx_len);   
	if(LMH_WaitForRx(spiHandle, rx_data, &rx_len, 11) == RV_OK)
	{
		data_cnt = 5;
...
int dwm_reset(SPI_HandleTypeDef* spiHandle)
{
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;
...
int dwm_ver_get(SPI_HandleTypeDef* spiHandle, dwm_ver_t* ver)
{
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;
	...
	tx_data[tx_len++] = 0;   
	LMH_Tx(spiHandle, tx_data, &tx_len);   
	if(LMH_WaitForRx(spiHandle, rx_data, &rx_len, 21) == RV_OK)
	{
		data_cnt = 3;      
		// read fw_version
...
int dwm_gpio_cfg_output(SPI_HandleTypeDef* spiHandle, dwm_gpio_idx_t idx, bool value)
{      
...
int dwm_gpio_cfg_input(SPI_HandleTypeDef* spiHandle, dwm_gpio_idx_t idx, dwm_gpio_pin_pull_t pull_mode)
{      
	if(LMH_CheckGPIOIdx(idx)!= RV_OK)
	{
		return RV_ERR;
...
int dwm_gpio_value_set(SPI_HandleTypeDef* spiHandle, dwm_gpio_idx_t idx, bool value)
{
	if(LMH_CheckGPIOIdx(idx)!= RV_OK)
	{
		return RV_ERR;
...
int dwm_gpio_value_get(SPI_HandleTypeDef* spiHandle, dwm_gpio_idx_t idx, bool* p_value)
{
	if(LMH_CheckGPIOIdx(idx)!= RV_OK)
	{
		return RV_ERR;
	...
	tx_data[tx_len++] = idx & 0xff;  
	LMH_Tx(spiHandle, tx_data, &tx_len);   
	if(LMH_WaitForRx(spiHandle, rx_data, &rx_len, 6) == RV_OK)
	{
...
int dwm_gpio_value_toggle(SPI_HandleTypeDef* spiHandle, dwm_gpio_idx_t idx)
{
	if(LMH_CheckGPIOIdx(idx)!= RV_OK)
	{
		return RV_ERR;
...
int dwm_status_get(SPI_HandleTypeDef* spiHandle, dwm_status_t* p_status)
{
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;
...
int dwm_int_cfg(SPI_HandleTypeDef* spiHandle, uint8_t value)
{        
	uint8_t tx_data[DWM1001_TLV_MAX_SIZE];
	uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
	uint16_t rx_len;
